...
public:
	...
	void Infix(Node* node) {
		...
		//연산자인 노드의 left에 우선순위가 더 낮은 연산자가 있다면 괄호 필요
		//숫자면 출력, 연산자면 재귀호출
		...
		//아래는 필요한 부분만 괄호 만드는 코드

		// if(IsDigit(node->item)){
		// 	return;
		// }
		
		// if(IsDigit(left->item)){
		// 	cout << left->item;
		// }
		// else{
		// 	if(Prec(left->item) < node->item){
		// 		cout << '(';
		// 		Infix(left);
		// 		cout << ')';
		// 	}
		...
		// if(IsDigit(right->item)){
		// 	cout << right->item;
		// }
		// else{
		// 	if(Prec(right->item) > node->item){
		// 		cout << '(';
		// 		Infix(right);
		// 		cout << ')';
		// 	}
