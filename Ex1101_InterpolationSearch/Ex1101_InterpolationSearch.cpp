...
int InterpolationSearch(int arr[], int low, int high, int x)
{
	...
	if (low <= high && x >= arr[low] && x <= arr[high])
	{
		...
		int pos = low + int(((float(x) - arr[low]) / (arr[high] - arr[low])) * float(high-low));
...
int iterInterpolationSearch(int arr[], int low, int high, int x){
	...
	if(low <= high && x >= arr[low] && x <= arr[high]){
		...
		while(pos >= low && pos <= high){
			...
			pos = low + int(((float(x) - arr[low]) / (arr[high] - arr[low])) * float(high-low));
		}
	}
	return -1;
}
...
int main()
{
	...
	for (auto x : { 10, 18, 33, 47, -1, 99 })
	{
		...
		int index = iterInterpolationSearch(arr, 0, n - 1, x);
