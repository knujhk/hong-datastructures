...
public:
	...
	void Resize() // 2배씩 증가
	{
		...
		T *tempQ = new T[capacity_ * 2];
		//front < rear
		// if(front_ < rear_){
		// 	memcpy(tempQ+front_+1,queue_+front_+1,(rear_ - front_) * sizeof(T));
		// }
		// else if(front_ > rear_){
		// 	memcpy(tempQ+front_+1,queue_+front_+1,(capacity_-(front_+1)) * sizeof(T));
		// 	memcpy(tempQ+capacity_,queue_,(rear_ + 1) * sizeof(T));
		// 	//최종 rear 인덱스 n이라 하면 n-capacity+1 = rear+1
		// 	rear_ = rear_ + capacity_;
		// }
		if(front_ < rear_){
		 	memcpy(tempQ+1,queue_+front_+1,(rear_ - front_) * sizeof(T));
		}
		else if(front_ > rear_){
			memcpy(tempQ+1,queue_+((front_+1)%capacity_),(capacity_-(front_+1)) * sizeof(T));
			memcpy(tempQ+1+(capacity_-(front_+1)),queue_,sizeof(T) * (rear_ + 1));
		}
	...
	void Enqueue(const T& item) // 맨 뒤에 추가, Push()
	{
		...
		queue_[rear_] = item;	//T가 사용자 정의 클래스면 복사 대입 연산자 오버로딩 필요
	}
