...
int main()
{
	...
	cout << h.Top() << endl;  // 중증도가 높은 Hulk 먼저 치료
	h.Pop();

	cout << h.Top() << endl;  // 중증도가 동일하지만 먼저 도착한 Ironman
	h.Pop();

	h.Push({ 2, 3, "Blue Beetle" });

	cout << h.Top() << endl;  // 늦게 도착했지만 중증도가 높은 Blue Beetle
	h.Pop();

	cout << h.Top() << endl;  // 마지막으로 Nick Fury
	h.Pop();
